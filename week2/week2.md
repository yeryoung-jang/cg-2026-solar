...
Task 1의 실제 비율을 유지하면서 전체 배치를 -1~1 범위 안에 넣기 위해 지구, 달, 인공위성에 공통으로 `1/390000`의 균등 크기 변환을 추가했습니다. 가장 먼 달의 바깥쪽까지의 거리가 386140km이므로 이보다 조금 큰 390000으로 나누면 약 0.9901이 되어 범위 안에 들어옵니다. 축 범위는 세 축 모두 1로 설정했습니다. 새로운 크기 변환은 세 물체 모두 카드 배치의 가장 왼쪽에 넣었습니다. 오른쪽부터 적용되기 때문에 Task 1의 변환이 적용된 뒤 물체의 크기와 지구에서 떨어진 거리가 함께 줄어들도록 설정한 것입니다. 같은 배율을 사용해 세 물체 사이의 실제 크기와 거리 비율도 유지했습니다. 지구의 반지름 6371, 달의 거리 384400과 반지름 1740, 위성의 거리 7056과 몸체 크기를 맞추는 배율 `0.002/1.2`는 그대로 두었습니다. 달과 위성의 공전 주기를 정한 회전식, 지구를 향하도록 넣은 `Rz(180)`, 위성 궤도를 기울이는 `Rx(98.1)`도 유지했습니다. 따라서 Task 1의 배치와 운동을 유지하면서 장면 전체의 크기만 줄였습니다.
...
Task 1과 Task 2에서는 실제 비율을 유지했지만 달까지 한 화면에 담으니 지구가 작게 보이고 인공위성의 형태는 거의 보이지 않았습니다. 상단의 위치값으로 움직임은 확인할 수 있었지만 화면으로 직접 인공위성이 어떻게 움직이는지 살펴보기는 어려웠습니다. 실제 크기와 거리의 거리의 차이를 정확히 보여주는 데에는 실제 비율이 의미가 있지만 각 물체의 모습과 공전하는 모습을 확인하기에는 불편하다고 생각했습니다. 따라서 공전하는 모습을 쉽게 보여주려면 크기와 거리를 조절할 필요가 있다고 판단했습니다.

### 2. 내가 선택한 표현 방법

지구, 달, 인공위성의 모습과 달과 위성이 지구 주위를 도는 모습을 한 화면에서 보여 주고 싶었습니다. Task 2의 실제 비율에서는 위성이 너무 작아 이런 모습을 확인하기 어려웠습니다. 그래서 물체의 크기와 거리를 조절하는 방법을 선택했습니다. 지구의 반지름을 1로 정하고 달과 인공위성은 실제 비율보다 상대적으로 크게 표현했습니다. 또한 달은 한 화면에 들어오도록 거리를 줄였고 위성은 지구와 구분하기 위해서 보기 쉽도록 지구에서 더 떨어뜨렸습니다. Task 2에서 사용한 공통 배율 `1/390000`은 세 물체에서 모두 제거하고 크기와 이동값을 다음과 같이 바꿨습니다.
...
Task 3에서는 세 물체의 모양과 공전하는 모습을 알아보기 쉽게 만드는 것을 목표로 했습니다. Task 2의 공통 배율 `1/390000`을 제거하고 물체의 크기와 거리를 각각 보기 편한 값으로 바꿨습니다. 여기서 사용한 값은 실제 km 단위가 아니라 화면 표현을 위해 정한 값입니다. 지구는 기준이 되도록 위치를 원점에 두고 반지름을 1로 설정했습니다. 달은 지구와 함께 한 화면에서 볼 수 있도록 이동값을 `(4, 0, 0)`으로 정하고 모양을 알아보기 쉽도록 반지름을 0.4로 설정했습니다. 이는 실제 비율보다 달을 지구에 가깝게 배치하고 상대적으로 크게 표현한 것입니다. 인공위성은 지구에서 떨어져 도는 모습을 보기 쉽도록 이동값을 `(2, 0, 0)`으로 설정했습니다. 크기 배율은 0.15로 수정해서 실제 비율에서 잘 보이지 않던 위성의 전체 모양을 알아볼 수 있도록 했습니다. 균등 크기 변환을 사용했기 때문에 제공된 위성 모형의 가로세로 비율은 유지됩니다. 달과 위성의 회전식은 그대로 두어 기존 공전 주기를 유지했습니다. 또한, 물체의 크기와 거리를 바꾸면서도 기존의 회전 방식은 유지하기 위해 지구를 향하도록 넣은 `Rz(180)`과 위성 궤도의 기울기를 정하는 `Rx(98.1)`도 유지했습니다. 가장 먼 달의 바깥쪽까지 거리는 `4 + 0.4 = 4.4`이므로 축 범위는 여유를 두어 세 축 모두 5로 설정했습니다. 이 배치는 세 물체를 구분하기 쉽게 하기 위한 것으로 실제 크기와 거리 비율은 유지되지 않습니다.

### 4. 실제 비율과 달라진 점

크기와 거리를 각각 조절했기 때문에 실제 비율은 유지되지 않습니다. 예를 들어, 실제 달까지의 거리는 지구 반지름의 약 60.3배이지만 이 화면에서는 4배로 줄였습니다. 따라서 이 화면으로 실제 거리나 물체의 크기를 판단하면 잘못 이해할 수 있습니다. 실제 수치를 비교할 때는 Task 1, 2의 설정을 참고하고 Task 3의 화면은 물체의 모습과 공전하는 모습을 설명하는 용도로 사용하는 것이 적합합니다.

![Task 3 결과](images/task3.png)

Task 2에서는 인공위성의 형태를 알아보기 어려웠지만 수정한 화면에서는 지구, 달, 인공위성의 모습을 한 화면에서 함께 확인할 수 있었습니다.

- 공유 링크: https://cg.catholic.ac.kr/~mgchoi/CG/demos/d02-transform-lab.html?d=%3D